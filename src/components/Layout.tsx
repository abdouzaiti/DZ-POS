...
export const Layout = ({ children, activeTab, setActiveTab }: { children: React.ReactNode, activeTab: string, setActiveTab: (tab: string) => void }) => {
  ...
  return (
    <div className="flex h-screen bg-white text-slate-900 overflow-hidden font-sans">
      ...
      <aside 
        className={cn(
          "bg-slate-50 border-r border-slate-200 flex flex-col transition-all duration-300 z-10",
          collapsed ? "w-16" : "w-48"
        )}
      >
        <div className="px-3 flex items-center justify-between border-bottom dark:border-slate-800 h-14">
          {!collapsed && (
        ...
        <header className="bg-white border-b border-slate-200 h-14 flex items-center justify-between px-4 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 text-slate-500 font-medium">
              ...
              <span className="flex items-center gap-1 text-[10px] bg-green-100 text-green-700 px-2 py-0.5 rounded-full uppercase">Poste 01</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-4">
              <button 
                ...
                className="px-3 py-1 text-sm font-bold bg-slate-100 dark:bg-slate-800 rounded hover:bg-slate-200 uppercase"
              >
              ...
              <div className="flex gap-1">
                <button className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded transition-colors text-slate-600">
                  <Maximize size={18} />
                </button>
                <button className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded transition-colors text-slate-600">
                  <Moon size={18} />
